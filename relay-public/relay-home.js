...
// src/security/e2ee.ts
var b64Variant = () => libsodium_wrappers_default.base64_variants.URLSAFE_NO_PADDING;
var encode = (bytes) => libsodium_wrappers_default.to_base64(bytes, b64Variant());
var decode = (value) => libsodium_wrappers_default.from_base64(value, b64Variant());
var text = (value) => libsodium_wrappers_default.from_string(value);
async function initializeCrypto() {
  await libsodium_wrappers_default.ready;
}
async function createPhoneHelloWithSigner(params) {
  await initializeCrypto();
  const ephemeral = libsodium_wrappers_default.crypto_kx_keypair();
  const unsigned = {
    version: 1,
  ...
  const hello = { ...unsigned, signature: encode(signature) };
  return {
    hello,
    state: {
      hello,
...
async function finishPhoneHandshake(params) {
  await initializeCrypto();
  ...
  if (!libsodium_wrappers_default.crypto_sign_verify_detached(
    decode(params.reply.signature),
    transcript,
    decode(params.bridgeIdentityPublicKey)
  )) throw new Error("Invalid bridge handshake signature");
  const sessionKeys = libsodium_wrappers_default.crypto_kx_client_session_keys(
...
var EncryptedChannel = class {
  ...
  encrypt(messageType, payload) {
    if (!/^[a-z][a-z0-9_.-]{0,63}$/.test(messageType)) throw new Error("Invalid encrypted message type");
    const counter = this.sendCounter++;
    const messageId = encode(libsodium_wrappers_default.randombytes_buf(16));
    const header = {
      version: 1,
    ...
    const ciphertext = libsodium_wrappers_default.crypto_aead_xchacha20poly1305_ietf_encrypt(
      plaintext,
      envelopeAad(header),
      null,
      nonce(counter),
  ...
  decrypt(envelope) {
    ...
    const plaintext = libsodium_wrappers_default.crypto_aead_xchacha20poly1305_ietf_decrypt(
      null,
    ...
    const parsed = JSON.parse(libsodium_wrappers_default.to_string(plaintext));
    this.receiveCounter = counter;
    return parsed;
  }
};
function phoneHelloTranscript(hello) {
  return frame(
...
function sessionTranscript(hello, bridgeEphemeralPublicKey) {
  return frame(
    "cchat-session-v1",
    hello.installationId,
    hello.phoneDeviceId,
...
function parseCounter(value) {
  ...
  if (counter > 0xffffffffffffffffn) throw new Error("Invalid encrypted counter");
  return counter;
}
...
var EncryptedTransport = class {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.socket = socket;
      const timeout = window.setTimeout(() => reject(new Error("Encrypted bridge connection timed out")), 15e3);
      socket.addEventListener("open", async () => {
        try {
          const initiated = await createPhoneHelloWithSigner({
            ...
            sign: async (message) => new Uint8Array(await crypto.subtle.sign("Ed25519", this.device.phoneIdentityPrivateKey, message.buffer))
          });
  ...
  async receive(raw, ready, reject) {
    try {
      ...
      if (message.frame.kind === "ready") {
        this.handshake = null;
        ready();
        return;
      }
      if (message.frame.kind !== "envelope" || !this.channel) return;
      const payload = this.channel.decrypt(message.frame.envelope);
